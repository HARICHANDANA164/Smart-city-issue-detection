...
### Authentication
- `POST /auth/request-otp` - request OTP for `register` or `login` using email or phone
- `POST /auth/register` - register (`citizen` or `authority`) with identifier + password + OTP
- `POST /auth/login` - login with identifier + password + OTP
...
1. Enter email/phone and click **Get OTP**.
2. Sign up as **citizen** with OTP and create an issue.
3. Sign up as **authority** with OTP.
4. Login as authority with OTP and update issue status (`Pending`, `Not Started`, `Completed`) with optional resolution image/comment.
5. Verify the common dashboard and analytics update for everyone.
